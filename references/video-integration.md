...
> **Looking for the no-recording path?** If the user picked Q6 = (b) "Claude builds a fake-data animation" in Phase 1, the demo slide is generated by Claude as an animation — no video file required. See `references/live-demo-archetypes.md` (6 archetypes, elicitation questions, building blocks) and `assets/live-demo-template.html` (scaffold). This file (`video-integration.md`) only covers the embedded-recording path (Q6 = a/d).
